...
#PBS -J 0-5:1
#PBS -o /scratch_lg/image-content/ameyasd/sax_benchmarks/stdout
#PBS -e /scratch_lg/image-content/ameyasd/sax_benchmarks/stderr
...
# Load configuration.
with open(SCRIPTS_DIR + 'secondary_scripts/hpc_config_sax.yaml', 'r') as config_file_object:
    config = yaml.safe_load(config_file_object)
...
if mode == 'eval':
    if not os.path.exists(RESULTS_DIR + filename_no_ext):
        os.makedirs(RESULTS_DIR + filename_no_ext)
